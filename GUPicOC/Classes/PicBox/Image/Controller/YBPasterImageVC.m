...
- (NSArray *)imageArray
{
    if (!_imageArray) {
        ...
        for(int i = 0; i < 12; i++){
            UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"Frame%d", i+1]];
...
- (YBFilterScrollView *)filterScrollView
{
    if (!_filterScrollView) {
        ...
        //NSArray *titleArray = @[@"Starry",@"Cubist",@"Feathers",@"Mosaic",@"Candy",@"瑞华",@"淡雅",@"酒红",@"青柠",@"浪漫",@"光晕",@"蓝调",@"梦幻",@"夜色"];
...
- (void)setupUI
{
    ...
    NSArray *array = @[@"滤镜",@"风格",@"编辑",@"文字",@"相框",@"贴图"];
    for (int i = 0; i < array.count; i ++)
    {
        CGFloat perButtonW = FULL_SCREEN_W/6;
    ...
    UIView *lineView = [[UIView alloc]initWithFrame:CGRectMake(0, bottomButtonH - 3, lineViewW, 3)];
...
//    CGFloat lineViewX = sendW * currentIndex;
    CGFloat lineViewCenterX = sender.gyl_centerX;
    CGFloat lineViewY = bottomButtonH - lineViewH;
//    CGFloat lineViewW = sendW/2;
    CGFloat lineViewW = sender.titleLabel.gyl_width;
    [UIView animateWithDuration:.5 animations:^{
//        self.lineView.frame = CGRectMake(lineViewX, lineViewY, lineViewW, lineViewH);
        self.lineView.gyl_centerX = lineViewCenterX;
        self.lineView.gyl_y = lineViewY;
        self.lineView.gyl_size = CGSizeMake(lineViewW, lineViewY);
    }];
}
...
- (void)setRightButton
{
    [self initRightButtonWithButtonW:50 buttonH:50 title:@"保存" titleColor:[UIColor orangeColor] touchBlock:^(UIButton *rightButton) {
    }];
